    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 space-y-4">
          <p className="font-medium text-slate-700">Formulário de Nova Empresa (Mock)</p>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 space-y-3">
            ...
            <p className="text-sm"><strong className="text-slate-600">Razão Social:</strong> {data.razao_social || data.name || "-"}</p>
            <p className="text-sm"><strong className="text-slate-600">Nome Fantasia:</strong> {data.nome_fantasia || "-"}</p>
            <p className="text-sm"><strong className="text-slate-600">CNPJ:</strong> {data.cnpj || "-"}</p>
            <p className="text-sm"><strong className="text-slate-600">Setor:</strong> {data.setor || "-"}</p>
          </div>

          <div className="bg-slate-50 p-6 rounded-lg border border-slate-200 space-y-4">
            <h2 className="text-base font-semibold text-slate-800 flex items-center gap-2">
              <span>🏢</span> URL Amigável da Empresa
            </h2>
            <p className="text-xs text-slate-500">
              Personalize o link público da empresa. O UUID interno permanecerá inalterado.
            </p>
          ...
          <div className="flex gap-4">
            <Link href="/taxmanagers/pessoas/novo">
              <span className="inline-block bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded text-xs font-medium cursor-pointer transition">
                + Adicionar Pessoa/Lead
              </span>
            </Link>
            <Link href={`/taxmanagers/negocios/novo?company_id=${data.id}`}>
              <span className="inline-block bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded text-xs font-medium cursor-pointer transition">
                + Criar Negócio para esta Empresa
              </span>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-red-50 text-red-700 p-4 rounded border border-red-200 text-sm">
          Empresa não encontrada com o identificador: <strong>{routeParam}</strong>
        </div>
      )}
    </div>
  );
